...
import 'package:flutter_clean_bloc_template/core/utils/constants.dart';
...
class Utils {
  static int syncWeekDays(int day) {
    if (sharedPreferencesManager.getFirstDayOFWeek() == 2) {
      if (day == 1) {
        return 7;
      } else {
        return day - 1;
  ...
  static Future<String?> refreshToken() async {
    ...
    //   URLPath.BASE_URL_API + URLPath.RefreshToken,
    //   data: jsonEncode({
    //     "RefreshToken": sharedPreferencesManager.getRefreshToken(),
    //   }),
    // );
    // if (RefreshTokenModel.fromJson(refreshToken.data).accessToken != null) {
    //   sharedPreferencesManager
    //       .setToken(RefreshTokenModel.fromJson(refreshToken.data).accessToken!);
    //   sharedPreferencesManager.setRefreshToken(
    //       RefreshTokenModel.fromJson(refreshToken.data).refreshToken!);
    //   return RefreshTokenModel.fromJson(refreshToken.data).accessToken!;
    // } else {
  ...
  static List<Color> getCardColor(int cardTypeID, bool isPending, bool isDone) {
    List<Color> color = [];
    switch (cardTypeID) {
      case 1:
        color.add(CustomColor.green);
        break;
      case 2:
        color.add(CustomColor.red);
        break;
      case 3:
        color.add(CustomColor.purple);
        break;
    }
    if (isPending) {
      color.add(CustomColor.lightBlue);
    } else {
      color.add(color[0]);
    }
    if (isDone) {
      color.add(CustomColor.yellow);
    } else {
      color.add(color[0]);
    }
    return color;
